 ...
 * Run once to prepare the sheet structure. Uses config.gs (CAMPAIGN_SHEET_COLUMNS, HEADERS_EMAIL_CAMPAIGN).
 */
function setupSheetColumns() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var report = [];

  var campaignSheetName = CAMPAIGN_SHEET_COLUMNS.invitation.sheetName;
  var campaignLastCol = HEADERS_EMAIL_CAMPAIGN.length;
  var campaignSheet = ss.getSheetByName(campaignSheetName);
  if (campaignSheet) {
    var lastCol = campaignSheet.getLastColumn();
    if (lastCol < campaignLastCol) {
      campaignSheet.insertColumnsAfter(lastCol, campaignLastCol - lastCol);
      report.push(campaignSheetName + ': added ' + (campaignLastCol - lastCol) + ' columns');
    }
    var reminderStart = CAMPAIGN_SHEET_COLUMNS.reminder.sent;
    var reminderHeaders = [
    ...
    // getRange(row, column, numRows, numColumns) - 4th arg is number of columns, not end column
    campaignSheet.getRange(1, reminderStart, 1, reminderHeaders.length).setValues([reminderHeaders]);
    report.push(campaignSheetName + ': set Reminder headers (cols ' + reminderStart + '-' + (reminderStart + reminderHeaders.length - 1) + ')');
  } else {
    report.push(campaignSheetName + ': sheet not found');
  }

  var approvedSheetName = CAMPAIGN_SHEET_COLUMNS.confirmation.sheetName;
  var confirmationPixelCol = CAMPAIGN_SHEET_COLUMNS.confirmation.pixelUrl;
  var approvedSheet = ss.getSheetByName(approvedSheetName);
  if (approvedSheet) {
    var lastColApproved = approvedSheet.getLastColumn();
    if (lastColApproved < confirmationPixelCol) {
      approvedSheet.insertColumnsAfter(lastColApproved, confirmationPixelCol - lastColApproved);
      report.push(approvedSheetName + ': added ' + (confirmationPixelCol - lastColApproved) + ' columns');
    }
    var confSentCol = CAMPAIGN_SHEET_COLUMNS.confirmation.sent;
    var confirmationHeaders = [
      ...
      'Link Clicked', 'First Click At',
      'Confirmation_Pixel_URL'
    ];
    // getRange(row, column, numRows, numColumns) - 4th arg is number of columns
    var numConfCols = confirmationPixelCol - confSentCol + 1;
    approvedSheet.getRange(1, confSentCol, 1, numConfCols).setValues([confirmationHeaders]);
    report.push(approvedSheetName + ': set Confirmation headers (AA-AH)');
  } else {
    report.push(approvedSheetName + ': sheet not found');
  }
...
function clearFalseTrackingData() {
  ...
  const data = sheet.getDataRange().getValues();
  const inv = CAMPAIGN_SHEET_COLUMNS.invitation;
  const emailSentCol = inv.sent;
  const emailOpenedCol = inv.opened;
  const firstOpenCol = inv.firstOpenAt;
  const openCountCol = inv.openCount;
  const linkClickedCol = inv.clicked;
  const firstClickCol = inv.firstClickAt;
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const rowNumber = i + 1;
    const email = row[inv.email - 1];
    const emailSent = row[emailSentCol - 1];

    if (emailSent !== 'Yes') {
      const hasTracking =
        row[emailOpenedCol - 1] === 'Yes' ||
        row[linkClickedCol - 1] === 'Yes' ||
        (row[openCountCol - 1] && Number(row[openCountCol - 1]) > 0);

      if (hasTracking) {
        sheet.getRange(rowNumber, emailOpenedCol).setValue('No');
        sheet.getRange(rowNumber, firstOpenCol).setValue('');
        sheet.getRange(rowNumber, openCountCol).setValue(0);
        sheet.getRange(rowNumber, linkClickedCol).setValue('No');
        sheet.getRange(rowNumber, firstClickCol).setValue('');
...
function hideTrackingURLColumns() {
  ...
  var inv = CAMPAIGN_SHEET_COLUMNS.invitation;
  sheet.hideColumns(inv.pixelUrl, 2);
...
function showTrackingURLColumns() {
  ...
  var inv = CAMPAIGN_SHEET_COLUMNS.invitation;
  sheet.showColumns(inv.pixelUrl, 2);
